...
/**
 * @author TestJas
 * @description 针对表【table_dispatcher_info】的数据库操作Service实现
 * @createDate 2022-06-13 08:52:13
 */
@Service
public class table_dispatcher_infoServiceImpl extends ServiceImpl<table_dispatcher_infoMapper, table_dispatcher_info>
        implements table_dispatcher_infoService {
    @Autowired
    private com.lxy.mapper.table_userMapper table_userMapper;
    @Autowired
    private com.lxy.mapper.table_user_infoMapper table_user_infoMapper;
    @Autowired
    ...
    public ResultsPack updateUser(int id, String name, String prename, String description) throws Exception {
        UpdateWrapper<table_dispatcher_info> uw = new UpdateWrapper();//更新操作
        if (name == null || name.equals("") || name.equals(prename)) {
            ...
            uw.eq("uid", id);
            table_dispatcher_info tdi = table_dispatcher_infoMapper.selectById(id);//根据id查用户信息
            table_dispatcher_infoMapper.update(tdi, uw);
            ...
            if (table_userMapper.selectOne(qw) != null) {//判断是否存在这个用户,如果存在
                if (description==null){
                    return new ResultsPack(false, "修改失败，用户名已存在~");
                }
                uw.set("description", description);
                uw.eq("uid", id);
                table_dispatcher_info tdi = table_dispatcher_infoMapper.selectById(id);//根据id查用户信息
            ...
            if (description==null){
                ...
                uw.eq("uid",id);
                table_dispatcher_info tui = table_dispatcher_infoMapper.selectById(id);
                table_dispatcher_infoMapper.update(tui, uw);//更新
                return new ResultsPack(true, "修改成功~");
            }
            uw.set("username", name);
            uw.set("description",description);
            uw.eq("uid", id);
            table_dispatcher_info tdi = table_dispatcher_infoMapper.selectById(id);//根据id查用户信息
            table_dispatcher_infoMapper.update(tdi, uw);//更新
